...
Antes de subir nada, Firebase **importa el módulo** para descubrir qué funciones exporta, y da **10 segundos** para ello. Ese presupuesto es la razón de que `apps/backend/src/functions.ts` construya el contenedor en la primera petición y no al cargar el módulo: hacerlo al cargar llamaba a `applicationDefault()`, que busca credenciales hasta consultar el servidor de metadatos, inexistente en el contenedor de análisis. Se agotaba sola tras unos nueve segundos y el despliegue pasaba por poco, hasta que dejó de pasar.

El coste se trasladó a la primera petición de cada instancia fría, que es lo que Firebase recomienda. La contrapartida es que en la salida del despliegue **ya no aparecen los logs de arranque** con la configuración de producción: el análisis solo registra los handlers. Para verificar los adaptadores elegidos hay que mirar los logs de la función después de la primera petición.
